...
int rop_string_set(redisContext *conn,const char *key, const char* value)
{
    ...
    if(strcmp(reply->str,"OK")==0)
    {	
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[+][GMS_REDIS]set %s %s SUCCESS!\n", key,value);
        freeReplyObject(reply);
        return 0;
    }
    else
...
int rop_string_get(redisContext *conn,const char* key,char *value)
{
    ...
    value[reply->len] = '\0';
    LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[+][GMS_REDIS]get %s SUCCESS!\n", key);
END:
    freeReplyObject(reply);
    return retn;
}
...
int rop_hash_set(redisContext *conn,const char* key,const char* field, const char* value)
{
    ...
    {	
        retn = 1;
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]HSET %s %s %s succ", key,field,value);
        goto END;
    }
